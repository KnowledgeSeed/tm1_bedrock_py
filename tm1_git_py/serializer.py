    ...
    for dim in dimensions:
        for _hierarchy in dim.hierarchies:
            ...
            for _subset in _hierarchy.subsets:
                ...
                with open(subsets_dir + '/' + _subset.name+'.json', 'w') as subset_file:
                    subset_file.write(_subset.as_json())
            with open(hierarchy_dir + '/' + _hierarchy.name+'.json', 'w') as hierarchy_file:
...
def serialize_cubes(cubes: List[Cube], cubes_dir):
    for cube in cubes:
        if cube.rule:
            with open(cubes_dir + '/' + cube.name+'.rules', 'w') as rule_file:
        ...
        if cube.views:
            ...
            for _mdxview in cube.views:
                with open(views_dir + '/' + _mdxview.name+'.json', 'w') as mdxjson_file:
                    mdxjson_file.write(_mdxview.as_json())

                with open(views_dir + '/' + _mdxview.name+'.mdx', 'w') as mdx_file:
                    mdx_file.write(_mdxview.mdx)
            if cube.rule:
                with open(cubes_dir + '/' + cube.name+'.rules', 'w') as rule_file:
                    rule_file.write(cube.rule)

        with open(cubes_dir + '/' + cube.name+'.json', 'w') as cube_file:
            cube_file.write(cube.as_json())


def serialize_processes(processes: List[Process], process_dir):
    for process in processes:
        with open(process_dir + '/' + process.name + '.ti', 'w') as processti_file:
            processti_file.write(process.ti.ti_as_string())

        with open(process_dir + '/' + process.name + '.json', 'w') as processjson_file:
            processjson_file.write(process.as_json())


def serialize_chores(chores: List[Chore], chores_dir):
    for chore in chores:
        with open(chores_dir + '/' + chore.name + '.json', 'w') as chore_file:
