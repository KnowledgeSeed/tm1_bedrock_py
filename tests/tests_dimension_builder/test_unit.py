...
test_data_sql_format_1 = [
    (EXPECTED_DF, sql_query, dtype_mapping),
    (EXPECTED_DF_MANDATORY_ONLY, sql_query_m_o, dtype_mapping_m_o)
]
@pytest.mark.parametrize("expected_df, query, dtype", test_data_sql_format_1)
def test_read_sql_parent_child_to_df(expected_df, query, dtype):
    ...
    try:
        ...
        cursor.executemany(
            "INSERT INTO dbo.Dim_Product_Edges (Parent, Child, ElementType, Weight, Hierarchy) VALUES (?, ?, ?, ?, ?);",
            rows,
        )
        connection.commit()
...
test_data_yaml_format_1 = [
    ...
    (EXPECTED_DF, "test_read_yaml_parent_child_to_df_aliases", yaml_aliases)
]
...
def test_read_yaml_parent_child_to_df(expected_df, template_key, col_spec):
    ...
    expected_dataframe = pd.DataFrame(expected_df)
    dataframe = read_source_to_df(source=source, source_type="yaml", template_key=template_key, column_aliases=col_spec)
    pd.testing.assert_frame_equal(dataframe, expected_dataframe)
